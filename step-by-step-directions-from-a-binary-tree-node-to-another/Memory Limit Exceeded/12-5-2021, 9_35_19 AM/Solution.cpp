...
public:
    ...
    void finds(TreeNode* lca,int st,int dt,bool &f,bool &fd,string r,string t){
        ...
        finds(lca->left,st,dt,f,fd,r+'U',t+'L');
        finds(lca->right,st,dt,f,fd,r+'U',t+'R');
    }
    void findd(TreeNode* lca,int dt,bool &f,string t){
        if(!lca || f) return;
        if(lca->val==dt){
            f=true;
            d=t;
        ...
        findd(lca->left,dt,f,t+'L');
        findd(lca->right,dt,f,t+'R');
    }
